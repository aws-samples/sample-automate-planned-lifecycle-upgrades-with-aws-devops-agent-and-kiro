   ...
   → EventBridge Rule (08:00 UTC cron) → Skill Review Lambda
       ...
       → Investigation Completed event → Trigger Lambda:
         IF changes found:
           ...
           └─→ Publishes to SNS (eks-skill-update-notifications topic)
...
- **Log groups use default encryption, not a customer-managed key.** Neither `EventBridgeLogGroup` nor the implicit Lambda log groups specify `KmsKeyId`, so they fall back to the AWS-owned key — the template documents this as an accepted POC trade-off via `cfn_nag`/`checkov` suppressions. The contents are operational metadata — cluster names and ARNs, task and execution IDs, and the full matched EventBridge event — rather than secrets, but a regulated environment will want `KmsKeyId` set explicitly. (The Secrets Manager secrets *are* encrypted with a customer-managed key; this gap is log groups only.)
...
- **The agent's read access is account-wide.** The investigation role grants an enumerated set of read-only EKS, CloudFormation, and CloudTrail actions (`eks:DescribeCluster`, `eks:DescribeAddon`/`ListAddons`, `eks:DescribeAddonVersions`, `eks:DescribeNodegroup`/`ListNodegroups`, `eks:DescribeUpdate`/`ListUpdates`, `cloudformation:DescribeStacks`/`DescribeStackEvents`/`ListStackResources`/`GetTemplate`, `cloudtrail:LookupEvents`) against `Resource: '*'`, since the cluster to investigate isn't known until a Health event arrives. Nothing here can mutate infrastructure, but narrow the resource scope before pointing this at an account whose stack templates you don't want the agent reading.

Every path ends at a pull request a human must review and merge. Deploying the merged
change is a separate manual trigger (`eks-deploy.yml`, Step 8) — nothing reaches the
...
| Path | Purpose |
|---|---|
| `bin/iteration3.ts` | CDK app entry point (name suffix, investigation tag, cdk-nag aspect) |
| `lib/iteration3-stack.ts` | EKS cluster, node group, addons, ALB controller Helm chart |
| `test/iteration3.test.ts` | Jest test asserting `cdk synth` passes with cdk-nag active |
| `devops-agent-space.yaml` | CFN template: single agent space, IAM, EventBridge rules, four inline Lambdas, SNS topics, KMS key, secrets |
| `bootstrap.sh` / `cleanup.sh` | One-shot deploy and teardown of both stacks |
...
| `skills/eks-upgrade-planning/SKILL.md` | AWS DevOps Agent skill: 9-step upgrade investigation (scoped to Incident RCA) |
...
- GitHub PAT — see [Step 4](#step-4-create-and-store-the-github-pat) for the exact permissions
- Kiro CLI API key (for GitHub Actions)
- AWS Command Line Interface (AWS CLI) configured with a profile that has admin access
- `node`, `npm`, and `npx` on your PATH (`bootstrap.sh` checks for these and exits if any is missing)
- `zip` / `unzip`, for packing the skill archives in Step 7
...
3. `cdk deploy` — VPC, EKS cluster (v1.30), an AL2023 managed node group, 3 managed addons (vpc-cni, kube-proxy, coredns), and the AWS Load Balancer Controller Helm chart (v1.10.0) (~20 min)
4. Builds and uploads the `devops-agent-sdk` Lambda layer (botocore service model JSON for the `devops-agent` API — see `lambda-layers/devops-agent-sdk/README.md`), then `aws cloudformation deploy` — DevOps Agent space, AWS Identity and Access Management (IAM) roles, EventBridge rules, four Lambda functions (Health, Trigger, Failure, Skill Review), the Lambda layer, two SNS topics, a KMS key, a dead-letter queue, and Secrets Manager secrets. The CFN template is over CloudFormation's 51,200-byte inline limit (mostly because the Lambdas ship inline), so `bootstrap.sh` passes `--s3-bucket` to `aws cloudformation deploy` and the CLI auto-uploads the template to the CDK assets bucket under `cloudformation-templates/` before invoking the API. No new bucket is provisioned
...
- Dropped events log which deployment owns the stack, so a "why didn't my investigation open?" question is one log query away:
  ...
  ```
  Ignoring rollback for stack EksUpgradePocStack-bob: this deployment owns
  EksUpgradePocStack-alice. Another deployment's failure detector handles it.
  ```
...
6. Choose **Generate**. The console shows two values **once**:
   ...
   Copy both immediately — you can't retrieve the secret later. If you lose it, regenerate the webhook and redo the "Store the credentials in Secrets Manager" step below.
...
This single webhook serves all three inbound paths — the Health Lambda, the Failure Lambda, and the Skill Review Lambda all POST to it. The triage skill (`eks-investigation-triage-rules`) is what keeps the resulting investigations from being linked to each other within the one space.

### Step 4: Create and store the GitHub PAT

Two Lambdas need the PAT. The Trigger Lambda calls `workflow_dispatch` on `eks-upgrade.yml` (CDK PR after upgrade-planning), `next-steps.yml` (code fix PR after mitigation), and `skill-update.yml` (skill PR after the daily review). The Skill Review Lambda uses the same token to read the current `SKILL.md` files through the GitHub Contents API. All three workflows live in this repo so a single PAT covers everything — but the scopes need to be repo-wide on Actions, not workflow-specific.

**Fine-grained token (recommended, single repo only)**
...
5. **Repository permissions** — grant these (Read and write):
   - **Actions**: Read and write (required for `workflow_dispatch` on `eks-upgrade.yml`, `next-steps.yml`, and `skill-update.yml`)
   - **Contents**: Read and write (needed by the Kiro CLI step to push a branch, and by the Skill Review Lambda to read `SKILL.md` files)
...
### Step 5: Subscribe to the mitigation notification SNS topic (optional but recommended)
...
### Step 6: Create and add the Kiro API key to GitHub

The GitHub Actions workflow runs Kiro CLI in headless mode, which needs an API key.

The `KIRO_MIN_VERSION` the workflows assert is `1.0.3`.
...
### Step 7: Upload skills and configure instructions
...
#### Step 7a: Configure Global Instructions
...
#### Step 7b: Configure Incident Mitigation instructions
...
#### Step 7c: Upload skills

Upload four skills. Each must be scoped to the correct agent type:

| Skill | Scope to | Purpose |
|---|---|---|
| `skills/eks-upgrade-planning/` | **Incident RCA** | 9-step EKS upgrade investigation producing a CDK Change Spec |
...
1. Zip the skill folder. The zips aren't committed — build them locally. Delete any
   existing zip first, because `zip -r` *adds to* an archive rather than replacing
   it, so re-running it over an existing zip leaves the stale `SKILL.md` inside next
   to the new one:
   ```bash
   cd skills
   for dir in */; do
...
### Step 9: Run the post-merge deploy workflow
...
### Step 10: Test the failure → mitigation path (optional)
...
**Option A: Trigger a real CloudFormation rollback**
...
### Step 11: Tear down the environment (optional)
...
4. **Delete Lambda log groups (optional, to stop CloudWatch charges).** CFN deletes the Lambdas but not their log groups — AWS retains them forever by default:
   ...
   ```bash
   for name in devops-agent-health-event devops-agent-trigger-upgrade \
               devops-agent-upgrade-failure-event devops-agent-skill-review-trigger; do
...
1. Deletes the CloudFormation stack (`DevOpsAgentStack`) — removes the agent space, IAM roles, EventBridge rules, all four Lambdas, the SNS topics, and Secrets Manager secrets
...
- The AWS OIDC IAM role you created for `eks-deploy.yml` (Step 9) — reusable across projects, delete from IAM if this was its only consumer
...
- CloudWatch log groups for the four Lambdas — see prerequisite 4 above if you want these gone
...
2. DevOps Agent follows the `eks-upgrade-planning` skill (Steps 1-9) → produces a `CDK Change Spec` recorded in journal records. The Trigger Lambda prefers the agent's `investigation_result` record over a slice of the raw transcript, falling back to slicing only if that record is absent
...
### How the workflows guard against a bad investigation
...
itself on a real-but-non-blocking observation. The case that motivated this rule was
the node group's AMI type — `NEEDS_REMEDIATION` with `BLOCKERS: Node group AMI type
must be migrated from AL2_x86_64 to AL2023_x86_64_STANDARD before node upgrade`. The
advice was sound, but the verdict was wrong: an AL2 AMI existed for the target
...
What makes this class of error recur is that an insight like `Amazon Linux 2
compatibility` reports `ERROR` on any AL2 node group whether or not a target AMI
exists — it is a lifecycle statement, not an upgrade-readiness verdict. Against a
cluster with AL2 nodes that insight is a permanent `ERROR`, so the same cluster can
produce opposite verdicts on two runs an hour apart.

This stack's node group now ships `AL2023_X86_64_STANDARD`, so that specific insight
no longer fires here — but the rule stays in the skill, because the same shape of
mistake is available on any advisory insight.
...
- `npm test` — Jest suite; asserts `cdk synth` passes with the cdk-nag AwsSolutions pack active
- `npx cdk deploy` — deploy this stack to your default AWS account/region
- `npx cdk diff` — compare deployed stack with current state
- `npx cdk synth` — emit the synthesized CloudFormation template

cdk-nag runs as an Aspect during synthesis (wired in `bin/iteration3.ts`), so any
unsuppressed AwsSolutions **Error**-level finding fails `cdk synth` — and therefore
fails the workflows' validation step before a PR is opened. Suppressions for
CDK-generated resources that can't be fixed live in the stack via `NagSuppressions`.
